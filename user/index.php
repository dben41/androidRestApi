...
if($_SERVER['REQUEST_METHOD'] == "GET")
{
	...
	elseif(count($resourceKeys) == 1) 
	{
	...
	} else if(count($resourceKeys) == 2){
		if($resourceKeys[0] == "username")
		{
			$user = null;
			try {
				//open db connection
				...
				//get the uid
				$usernameFromUrl = $resourceKeys[1];
				$stmt->bindValue(1, $usernameFromUrl);
				$stmt->execute();
		...
		} else if($resourceKeys[0] == "getClassUsers") { //returns a number
			$count = 0;
			try{ 

				//open db connection
				$DBH = openDBConnection();
				$stmt = $DBH->prepare("SELECT COUNT(role) AS studentCount FROM user WHERE class_id = ? AND role = 0;");
				//get the uid
				$usernameFromUrl = $resourceKeys[1];
				$stmt->bindValue(1, $usernameFromUrl);
				$stmt->execute();
		...
		} else if($resourceKeys[0] == "class") { //returns the whole class
			try{ 
				//open db connection
				$DBH = openDBConnection();
				$stmt = $DBH->prepare("SELECT uid, name, phone, email from user where  class_id = ? and role = 0;");
				//get the uid
				$usernameFromUrl = $resourceKeys[1];
				$stmt->bindValue(1, $usernameFromUrl);
				$stmt->execute();
